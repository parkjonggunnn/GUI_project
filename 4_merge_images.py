...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\danie\Documents\gui_project")  # 최초에 C:/ 로 경로를 보여줌
...
# 이미지 통합
def merge_image():
    # print(list_file.get(0, END)) # 모든 파일목록을 가지고 오기
    images = [Image.open(x) for x in list_file.get(0,END)]
    # size = size[0] : width, size[1] : height 
    # widths = [x.size[0] for x in images]
    # eights = [x.size[1] for x in images]
    ...
    #for img in images:
       # result_img.paste(img, (0,y_offset))
        #y_offset += img.size[1] # heigth 값 만큼 더해주기 (사진 밑에 붙이기 위해)
